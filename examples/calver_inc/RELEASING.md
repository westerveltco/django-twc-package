...
1. Create a new git branch off of `main` for the release.
   ...
   Prefer the convention `release-<version>`, where `<version>` is the next incremental version number (e.g. `release-v2026.1` for version 2026.1).
...
6. Draft a [new release](https://github.com/example_owner/calver-inc1-project/releases/new) on GitHub.

   Use the version number with a leading `v` as the tag name (e.g. `v2026.1`).
